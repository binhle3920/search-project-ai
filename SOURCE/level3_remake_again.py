...
def go_A_star(Map,MapLen,start,goal,monster_path,food_pos):
    ...
    while pacman_pos!=goal:
        ...
        if pacman_new_step in moveable_pos:
            ...
            path_A_star.pop(0)
    return Map,path_start_goal

def cell_can_go(Map,MapLen,pacman_pos): #can near monster
    cells=[]
    for v in range(4):
        temp=None
        if (pacman_pos[0] > 0) and (v == 0):
            temp = AddNode(pacman_pos, v)
        if (pacman_pos[1] > 0) and (v == 1):
            temp = AddNode(pacman_pos, v)
        if (pacman_pos[0] < MapLen[0]-1) and (v == 2):
            temp = AddNode(pacman_pos, v)
        if (pacman_pos[1] < MapLen[1]-1) and (v == 3):
            temp = AddNode(pacman_pos, v)
        if (temp!=None) and (Map[temp[0]][temp[1]]!=1) and (Map[temp[0]][temp[1]]!=3):
            cells.append(temp)
    return cells
...
def A_star_call(Map,MapLen,PacmanPos):
    ...
    while (True):
        ...
        if len(food_pos)==0:
            Map,food_pos=random_find_food(Map,MapLen,path_pacman,monster_path,explored_cells)
            if len(food_pos)==0:
                return path_pacman,monster_path
        Map,path_food = go_A_star(Map,MapLen, path_pacman[-1], food_pos[0],monster_path, food_pos)
        print('path_food',path_food)
        if len(food_pos)>0:
